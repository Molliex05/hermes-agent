...
# Noriven: lightweight sessions REST API (SQLite → JSON) for external UIs.
# Default on (SESSIONS_API=1). Port SESSIONS_API_PORT (default 9119). If you
# enable HERMES_DASHBOARD on the same port, set SESSIONS_API=0 or change ports.
case "${SESSIONS_API:-1}" in
    1|true|TRUE|True|yes|YES|Yes)
        echo "Starting Noriven sessions API on port ${SESSIONS_API_PORT:-9119} (background)"
